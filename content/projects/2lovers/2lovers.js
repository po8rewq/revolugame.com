(function () {
  ...
  LevelData.init = function () {
    ...
    LevelData.levels.push({
      file: 'map1',
      text: "You're controlling two connected lovers. Collect all the hearts to open the portal.",
    });
    LevelData.levels.push({
      file: 'map2',
      text: 'You can jump by pressing [UP]',
    });
    LevelData.levels.push({
      file: 'map3',
      text: 'Be careful not to fall into the traps!',
    });
    LevelData.levels.push({
      file: 'map5',
      text: 'Love is so powerful that the lovers are connected even worlds apart.',
    });
    LevelData.levels.push({
      file: 'map4',
      text: 'This switch seems to open/close the gate in the other world',
    });
    LevelData.levels.push({
      file: 'map6',
      text: 'Their love is more powerful than their fear...',
    });
    LevelData.levels.push({
      file: 'map7',
      text: 'She must be here to help him overcome all obstacles.',
    });
    LevelData.levels.push({
      file: 'map8',
      text: "Don't choose the easy path by drinking a potion",
    });
    LevelData.levels.push({
      file: 'endmap',
      text: 'But they found that worlds are finally connected',
    });
  };
  ...
  var TiledMap = function (pFile, pCellSize) {
    ...
    if (pCellSize > 0) this.tileSize = pCellSize;
    else this.tileSize = Reflect.field(json, 'tilewidth');
    ...
    while (_g4 < spawnData.length) {
      ...
      if (spawn.name == 'item')
        ...
        this.door = { x: Math.round(spawn.x), y: Math.round(spawn.y) };
        ...
        this.gates.push({
          ...
          inverted: spawn.type == 'inverted',
          visible:
            spawn.properties.visible == null ||
            spawn.properties.visible == 'true',
        });
  ...
  behaviors.KeyboardMovements.prototype = $extend(
    behaviors.BasicMovements.prototype,
    {
      ...
      update: function () {
        var directionX = 0;
        if (this._hero.isMovingLeft())
          directionX = -1 * (this._controlInverted ? -1 : 1);
        else if (this._hero.isMovingRight())
          if (this._controlInverted) directionX = -1;
          else directionX = 1;
        ...
        if (Math.abs(this.velocityY) > Math.abs(this._jumpSpeed))
          this.velocityY = this._jumpSpeed * this._gravity;
  ...
  behaviors.MapCollisions = function (
    pEntity,
    pData,
    pMapWidth,
    pTileSize,
  ...
  behaviors.MapCollisions.prototype = {
    ...
    update: function () {
      ...
      if (this.gravity == Gravity.INVERTED) this.check_ground_invertedGravity();
      else this.check_ground_normalGravity();
      if (
    ...
    check_ground_invertedGravity: function () {
      this.upy = Math.round(
        (this.entity.y - 1 - this._tileSize / 2) / this._tileSize
      );
      this.rightx = Math.round(
        (this.entity.x + this.entity.width - this._tileSize / 2) /
          this._tileSize
  ...
  entities.Door.prototype = $extend(age.display.Entity.prototype, {
    open: function () {
      ...
      this.visible = true;
      this._map.removeTileAt(
        Math.round(this._map.door.x / this._map.tileSize),
        Math.round(this._map.door.y / this._map.tileSize)
      );
      this._map.removeTileAt(
        Math.round(this._map.door.x / this._map.tileSize) + 1,
        Math.round(this._map.door.y / this._map.tileSize)
      );
      this._map.removeTileAt(
        Math.round(this._map.door.x / this._map.tileSize),
        Math.round(this._map.door.y / this._map.tileSize) + 1
      );
      this._map.removeTileAt(
        Math.round(this._map.door.x / this._map.tileSize) + 1,
        Math.round(this._map.door.y / this._map.tileSize) + 1
  ...
  entities.Gate = function (pX, pY, pId, pInverted, pDefaultState) {
    ...
    this.x = pX;
    this.y = pY;
    if (pDefaultState == null || pDefaultState == entities.GateState.CLOSED)
      this.close();
    else this.open();
    this.col = Math.round(pX / 16);
    this.line = Math.round(pY / 16);
    var img;
    if (pInverted) img = 'gateinverted';
    else img = 'gate';
  ...
  entities.Pic = function (pX, pY, pPosition, pInverted) {
    ...
    this.x = pX;
    this.y = pY;
    if (pPosition == 'top') this.position = entities.Position.TOP;
    else this.position = entities.Position.BOTTOM;
    this.inverted = pInverted;
    var img = 'pic';
    if (this.position == entities.Position.BOTTOM)
      if (pInverted) img = 'picInverted';
      else img = 'pic';
    else if (pInverted) img = 'pic';
    else img = 'picInverted';
    this.addImage(img, img, true);
  };
  ...
  entities.Player = function (pX, pY, pGravity, pAnim, pControlsInverted) {
    ...
    this.gravity = pGravity;
    this._movements = new behaviors.KeyboardMovements(
      this,
      Enums.getGravity(this.gravity),
      pControlsInverted
    );
  ...
  entities.Player.prototype = $extend(age.display.AnimatedEntity.prototype, {
    ...
    isMovingLeft: function () {
      ...
      return age.utils.GamepadSupport.direction(
        this.idControls,
  ...
  entities.Potion = function (pX, pY, pInverted, pPlayerPU, pPlayerAff) {
    ...
    var img;
    if (pInverted) img = 'potion1inverted';
    else img = 'potion1';
    this.addImage(img, img, true);
  };
  ...
  entities.Potion.prototype = $extend(age.display.Entity.prototype, {
    update: function () {
      if (
        !this.dead &&
        this.collidePoint(this._playerPU.x + 11, this._playerPU.y + 11)
      ) {
  ...
  entities.Switch = function (pX, pY, pInverted, pPlayer, pMap) {
    ...
    this.y = pY;
    var img;
    if (pInverted) img = 'switchinverted';
    else img = 'switch';
    this.addImage(img, img, true);
  ...
  entities.SwitchState = {
    __ename__: true,
    __constructs__: ['ACTIVE', 'UNAVAILABLE'],
  };
  ...
  managers.ItemsManager.getInstance = function () {
    if (managers.ItemsManager._instance == null)
      managers.ItemsManager._instance = new managers.ItemsManager();
  ...
  managers.ItemsManager.prototype = {
    ...
    checkCollisionsWithPics: function (pPlayer1, pPlayer2) {
      ...
      while (_g < _g1.length) {
        ...
        if (pic.inverted) {
          if (
            pic.position == entities.Position.BOTTOM &&
            pPlayer2.x + 11 >= pic.x &&
            pPlayer2.x + 11 <= pic.x + pic.width &&
            pPlayer2.y <= pic.y
          )
            return true;
          else if (
            pic.position == entities.Position.TOP &&
            pPlayer2.x + 11 >= pic.x &&
            pPlayer2.x + 11 <= pic.x + pic.width &&
            pPlayer2.y + pPlayer2.height >= pic.y + pic.height - 1
          )
            return true;
        } else if (
          pic.position == entities.Position.BOTTOM &&
          pPlayer1.x + 11 >= pic.x &&
          pPlayer1.x + 11 <= pic.x + pic.width &&
          pPlayer1.y + pPlayer1.height >= pic.y + pic.height - 1
        )
          return true;
        else if (
          pic.position == entities.Position.TOP &&
    ...
    addSwitchs: function (pMap, pPlayer1, pPlayer2) {
      ...
      while (_g < _g1.length) {
        var g = _g1[_g];
        ++_g;
        gate = new entities.Gate(
          g.x,
          g.y,
          g.id,
          g.inverted,
          g.visible ? entities.GateState.CLOSED : entities.GateState.OPENED
        );
      ...
      while (_g2 < _g11.length) {
        var s = _g11[_g2];
        ++_g2;
        swi = new entities.Switch(
          s.x,
          s.y,
          s.inverted,
          s.inverted ? pPlayer2 : pPlayer1,
          pMap
        );
        var _g21 = 0;
        var _g3 = this._gates;
  ...
  managers.UiManager = function () {
    ...
    this._instructions = new age.display.text.BasicText(
      '[SPACE] to continue',
      795,
      290
    );
    this._instructions.setStyle(
      'pixelade',
      25,
      '#FFF',
      false,
    ...
    this._text = new age.display.text.BasicText(
      ' ',
      400,
  ...
  states.GameState.prototype = $extend(age.display.State.prototype, {
    ...
    loadNextLevel: function (pForceText) {
      ...
      if (level == null) this._uiManager.addText(LevelData.endText, null);
      else {
        this._uiManager.addText(
          pForceText == '' ? level.text : pForceText,
          $bind(this, this.startLevel)
        );
        this._map = new TiledMap(level.file, 16);
        this._gameContainer.add(this._map);
        this._player1 = new entities.Player(
          this._map.player1.x,
          this._map.player1.y,
          Gravity.NORMAL,
          'player1'
        ...
        this._player2 = new entities.Player(
          this._map.player2.x,
          this._map.player2.y,
          this._map.player2.inverted ? Gravity.NORMAL : Gravity.INVERTED,
          this._map.player2.inverted ? 'player2inv' : 'player2',
          this._map.player2.inverted
        );
    ...
    update: function () {
      if (!this._paused) {
        ...
        if (
          this._player1.x <= -this._player1.width ||
          this._player1.x + this._player1.width >= 800 + this._player1.width ||
          this._player2.x <= -this._player2.width ||
          this._player2.x + this._player2.width >= 800 + this._player2.width
        ) {
          this.restartLevel("Don't try to escape this world!");
          return;
        }
        if (this._itemsManager.getItemsCount() > 0)
          this._itemsManager.checkCollisionsWithItems(
            this._player1,
            this._player2
          );
        else this._door.open();
        if (this._itemsManager.getPicsCount() > 0) {
          if (
            this._itemsManager.checkCollisionsWithPics(
              this._player1,
              this._player2
            )
          )
            this.restartLevel(
              'You just killed one lover... Try again, but be careful this time.'
            );
        }
        if (
          this._door.collidePoint(this._player1.x + 11, this._player1.y + 2)
        ) {
          this._player1.out = true;
          this._player1.visible = false;
        }
        if (
          this._door.collidePoint(this._player2.x + 11, this._player2.y + 20)
        ) {
  ...
  states.IntroState.prototype = $extend(age.display.State.prototype, {
    create: function () {
      ...
      this.add(title);
      var txt = new age.display.text.BasicText(
        'Game created for the Ludum Dare #30 in 48 hours by RevoluGame',
        400,
        130
      );
      txt.setStyle(
    ...
    update: function () {
      if (age.core.Input.pressed(32))
        age.core.Global.engine.switchState(new states.GameState());
      age.display.State.prototype.update.call(this);
    },
  ...
  var Enum = {};
  LevelData.endText =
    'And in the end they lived happily ever after - GAME OVER';
  Main.DEFAULT_FONT = 'pixelade';
  Main.URI = '/assets/img/portfolio/2lovers/';
